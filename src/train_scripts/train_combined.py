...
# from datasets.joint_img_dataset import JointImgDataset
from models.cnn_gru import CombinedCnnGru
...
def train_net(train_inputs, dataloaders, dataset_sizes, model, name):
    ...
    for epoch in range(train_inputs.epochs):
        ...
        for phase in ['train', 'val']:
            ...
            torch.save(model.state_dict(), '/data/backup_models/cnn_rnn_' + name + '/combined-v1/model_' + str(epoch) +  '.pt')           
...
def main():
    ...
    model = CombinedCnnGru(model_params)
    model.cuda()
    ...
    train_loader = utils.data.DataLoader(train_data, batch_sampler=rand_batch_sampler, num_workers=0)
    
    nrand_batch_sampler = NonRandomBatchSampler(data_source=val_data, batch_size=batch_size)
    val_loader = utils.data.DataLoader(val_data, batch_sampler=nrand_batch_sampler, num_workers=0)
